"""Module responsible for the MovieLens100k dataset."""
...
class MovieLens1M(Dataset):
    ...
    you have to manually set the preprocessing filters.::
        ...
        d = MovieLens1M('path/to/file', use_default_filters=False)
    ...
    DEFAULT_FILENAME = "ratings-1m.dat"
    """Default filename that will be used if it is not specified by the user."""
    
    @property
    def _default_filters(self) -> List[Filter]:
        """The default filters for the Movielens 1M dataset
    ...
    def _download_dataset(self):
        ...
        """
        DATASETURL = "https://files.grouplens.org/datasets/movielens/ml-1m.zip"

        # Download the zip into the data directory
        _fetch_remote(DATASETURL, os.path.join(self.path, "ml-1m.zip"))

        # Extract the ratings file which we will use
        with zipfile.ZipFile(os.path.join(self.path, "ml-1m.zip"), "r") as zip_ref:
            zip_ref.extract("ml-1m/ratings.dat", self.path)

        # Rename the ratings file to the specified filename
        os.rename(os.path.join(self.path, "ml-1m/ratings.dat"), self.file_path)
