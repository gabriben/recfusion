"""Module responsible for the MovieLens20M dataset."""
...
class MovieLens20M(Dataset):
    """Handles Movielens 20M dataset.

    All information on the dataset can be found at https://grouplens.org/datasets/movielens/20m/.
    ...
    you have to manually set the preprocessing filters.::
        ...
        from recpack.datasets import MovieLens20M
        d = MovieLens20M('path/to/file', use_default_filters=False)
    ...
    def _default_filters(self) -> List[Filter]:
        """The default filters for the Movielens 20M dataset
    ...
    def _download_dataset(self):
        ...
        """
        DATASETURL = "http://files.grouplens.org/datasets/movielens/ml-20m.zip"

        # Download the zip into the data directory
        _fetch_remote(DATASETURL, os.path.join(self.path, "ml-20m.zip"))

        # Extract the ratings file which we will use
        with zipfile.ZipFile(os.path.join(self.path, "ml-20m.zip"), "r") as zip_ref:
            zip_ref.extract("ml-20m/ratings.csv", self.path)

        # Rename the ratings file to the specified filename
        os.rename(os.path.join(self.path, "ml-20m/ratings.csv"), self.file_path)
