...
class MovieLens100K(Dataset):
    ...
    you have to manually set the preprocessing filters.::
        ...
        d = MovieLens100K('path/to/file', use_default_filters=False)
    ...
    def _download_dataset(self):
        ...
        """
        DATASETURL = "https://files.grouplens.org/datasets/movielens/ml-100k.zip"

        # Download the zip into the data directory
        _fetch_remote(DATASETURL, os.path.join(self.path, "ml-100k.zip"))

        # Extract the ratings file which we will use
        with zipfile.ZipFile(os.path.join(self.path, "ml-100k.zip"), "r") as zip_ref:
            zip_ref.extract("ml-100k/u.data", self.path)

        # Rename the ratings file to the specified filename
        os.rename(os.path.join(self.path, "ml-100k/u.data"), self.file_path)
