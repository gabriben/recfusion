...
class RecFusionBinInpaint(TorchMLAlgorithm):
    ...
    def __init__(
        ...
        self.M = M
        self.schedule_type = schedule_type
        self.jascha_bin_process = jascha_bin_process
    ...
    def get_beta_schedule(self, b_start, b_end, T, schedule_type):
        if schedule_type == 'quadratic':
            ...
            betas = np.repeat(b_start, T)
        elif schedule_type == 'Jascha':
            betas = np.array([])
            for t in range(T, 0, -1):
                betas = np.append(betas, 1 / (T - t + 1))
        return betas
    ...
    def _train_epoch(self, train_data: csr_matrix):
        ...
        # self.betas = [train_data.sum() / (train_data.shape[0] * train_data.shape[1])] * self.T
        ...
        n_batches =  len([b for b, _ in enumerate(get_batches(users, self.batch_size))])
        data_loader = get_batches(users, self.batch_size)
        ...
        for batch_idx, user_batch in enumerate(data_loader):
            ...
            self.Z = [make_noise(X, self.betas[0], Jascha = self.jascha_bin_process)]

            for i in range(1, self.T):
                self.Z.append(make_noise(self.Z[-1], self.betas[i], Jascha = self.jascha_bin_process))            
    ...
    def _compute_loss(
        ...
        for i in range(len(Z_hat)):
            KL_i = (log_normal_diag(self.Z[i], torch.sqrt(1. - self.betas[i]) * self.Z[i], torch.log(
                self.betas[i])) - log_normal_diag(self.Z[i], self.Z_hat[i], self.betas[i])).sum(-1)            
    ...
    def _batch_predict(self, X: csr_matrix, users: List[int]) -> csr_matrix:
        ...
        x = naive_sparse2tensor(active_users).to(self.device)
    ...
    def _evaluate(self, val_in: Matrix, val_out: Matrix) -> None:
        ...
        y = naive_sparse2tensor(val_out).to(self.device)
...
def make_noise(x, B, Jascha = True):
    if Jascha:
        p_x = torch.bernoulli(torch.zeros_like(x) + x * (1-B) + 0.5 * B)
        return x * p_x
    else:
        p = torch.bernoulli(torch.zeros_like(x) + B)
        return (1 - p) * x + p * (1 - x)
#
