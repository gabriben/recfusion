...
class RecFusionMLPT(TorchMLAlgorithm):
    ...
    def __init__(
        ...
        time_embedding_as_input: bool = False
        ...
        self.time_embedding_as_input = time_embedding_as_input
    ...
    def _batch_predict(self, X: csr_matrix, users: List[int]) -> csr_matrix:
        ...
        out_tensor = self.model_(in_tensor, t)
...
class MLPT(nn.Module):
    def __init__(self, depth, D, M, time_embedding_as_input):
        super().__init__()
        self.tx = time_embedding_as_input
        self.t_dim = 128
        self.m = nn.Sequential(
            *[nn.Linear(D, M), nn.PReLU()] +
            [nn.Linear(M, M), nn.PReLU()] * depth +
            [nn.Linear(M, D), nn.Tanh()])
        self.tm = nn.Linear(self.t_dim, D)
...
# B = 4
# self.t_emb = torch.rand(B, t_dim).cuda()
